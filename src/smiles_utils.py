...
def find_same_index(a,b):
    same_index = []
    for i in range(len(a)):
        for j in range(len(b)):
            if a[i] == b[j]:
                same_index.append(a[i])
    return same_index
...
def calc_similar(ref,mols):
    mollist = [] #fplist
    fp = AllChem.GetMorganFingerprintAsBitVect(ref,2)
    mollist.append(fp)
    fplist = [] #fplist
    ...
    for i in range(len(fplist)):
        a = DataStructs.DiceSimilarity(mollist[0],fplist[i])
        sim.append(a)
