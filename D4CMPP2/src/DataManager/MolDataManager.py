...
class MolDataManager:
    ...
    def generate_graph(self,col,graphs=None):
        ...
        if len(self._molecule_smiles[col]) < 10:
            ...
            iterator = tqdm(self._molecule_smiles[col], desc=f"Generating {col} graphs")
        for smi in iterator:
    ...
    def save_graphs(self,col):
        hash_smiles = torch.tensor([hash(smi) for smi in self._molecule_smiles[col]]) # hash the smiles to avoid the mismatch of the graphs and smiles
    ...
    def load_graphs(self,col):
        ...
        if len(self.molecule_graphs[col]) != len(self._molecule_smiles[col]):
            raise ValueError("Graphs and smiles are not matched")
        for i in range(len(self.molecule_graphs[col])):
            if hash(self._molecule_smiles[col][i]) != int(molecule_smiles['smiles'][i]): # check the hash of the smiles
                print(hash(self._molecule_smiles[col][i]),int(molecule_smiles['smiles'][i]), self._molecule_smiles[col][i])
                raise ValueError("Graphs and smiles are not matched")
            ...
            if hasattr(self, 'numeric_inputs') and self.numeric_inputs is not None:
                ...
                for col in self.numeric_input_columns:
                    if col in self.df.columns:
                        ...
                        numeric_inputs[col] = torch.zeros((len(self._molecule_smiles[self.molecule_columns[0]]), 1), dtype=torch.float32)
        else:
            numeric_inputs = None
    ...
    def split_data(self):
        ...
        if self.set is None:
            ...
            self.test_dataset = _subset_dataset(test_idx)
        else:
    ...
    def init_Dataloader(self,dataset=None,partial_train=1.0, shuffle=False):
        ...
        if partial_train and partial_train<1.0 and partial_train>0:
            subset_idx = np.random.choice(len(self.train_dataset),int(len(self.train_dataset)*partial_train),replace=False)
...
class MolDataManager_withSolv(MolDataManager):
    ...
    def init_temp_data(self,smiles,solvents,**kwargs):
        self.molecule_graphs={col:[] for col in self.molecule_columns}
        self._molecule_smiles={'compound':np.array(smiles),'solvent':np.array(solvents)}
        self.molecule_smiles={col:arr.copy() for col,arr in self._molecule_smiles.items()}
        self.valid_smiles={col:arr.copy() for col,arr in self._molecule_smiles.items()}
